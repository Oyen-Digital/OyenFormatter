...
static int selected_pt = -1, selected_fs = FS_NTFS, preselected_fs = FS_NTFS;
...
BOOL enable_HDDs = TRUE, enable_VHDs = TRUE, enable_ntfs_compression = FALSE, no_confirmation_on_cancel = FALSE;
...
BOOL zero_drive = FALSE, list_non_usb_removable_drives = TRUE, enable_file_indexing, large_drive = FALSE;
...
static void SetAllowedFileSystems(void)
{
	...
	case BT_NON_BOOTABLE:
		...
		*/
		...
		allowed_filesystem[FS_FAT16] = FALSE;
	...
	for (fs_index = 0; fs_index < FS_MAX; fs_index++) {
		...
		if (SelectedDrive.ClusterSize[fs_index].Allowed != 0) {
			...
			if (allowed_filesystem[fs_index]) {
				if ( (strstr(entry, "NTFS") != NULL) || (strstr(entry, "xFAT") != NULL) || (strstr(entry, "XFAT") != NULL) ) {
						IGNORE_RETVAL(ComboBox_SetItemData(hFileSystem,	ComboBox_AddStringU(hFileSystem, entry), fs_index));
				}
			}
		}
	}
...
//		SetWindowTextU(hLabel, SelectedDrive.proposed_label);
		SetWindowTextU(hLabel, "New Volume");
